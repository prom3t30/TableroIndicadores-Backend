...
//use App\Http\Resources\EjecucionIndicador;
use App\EjecucionIndicador;
use Illuminate\Http\Request;
use App\Indicadores;
use App\MetaPorLinea;
use App\Lineaprogramatica;
use App\MetaEsperadaEnLinea;
...
class MetaEsperadaEnLineaController extends Controller
{
    ...
    {
        Log::debug(response()->json($request->all()));
        $valores = array(
            "metaPorLinea" => array(),
            "mes" => array()
        );
        ...
        array_push($valores["mes"], EjecucionIndicador::select('valorejecucionesperada', 'valorejecucionrealizada')->where([['mes', '=', 'Diciembre'], ['year', '=', $request->year], ['metaXLinea_id', '=', $valores["metaPorLinea"][0]->id]])->orderby('created_at', 'DESC')->take(1)->toSql());
